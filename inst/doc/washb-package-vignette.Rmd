 ...
 * `'objectname$glmModel`  to return the glm model fit to be used with `predict()` to return model predictions of the outcome. Note that this is the glm model fit without `coeftest()` applied, so does not have robust standard errors, and should not be used for inference.   
 * `'objectname$glmModelRD` to return the risk differenc glm model fit to be used with `predict()` to return model predictions of the outcome. The same caveat as for `$glmModel` applies; the sandwich estimator function has not been applied to the model so the standard errors are incorrect.   
...
#BEN: I know theoretically these are asymptotically equivalent, but the estimates seem diff enough in the Bangladesh data... more than rounding errors. Should we encourage one function over the other?
...
The permutation test can also test for independence of $Y$ and $tr$, conditional on $W$, by applying the `washb_permute` function to the residuals of an adjusted model (adjusted for $W$, but not $tr$). The adjusted permutation test works better on outcomes with greater variability, so will be applied to the LAZ outcome here. 

#(BEN, WHY IS THIS?)
```{r, eval=T, include=T, comment=NA}
