...
####Input files:
 * __washb-bangladesh-diar.csv__
 * __washb-bangladesh-anthro.csv__  
 * __washb-bangladesh-enrol.csv__  
 ...
 * __sandwichSE__  Function to calculate the Huber Sandwich Estimator for robust standard errors. [Reference](http://www.stat.berkeley.edu/~census/mlesan.pdf)  
...
Diarrheal data with the above cleaning steps applied is available in the __"washb\_bd_diarClean.Rdata"__ file, accessed with `data(washb_bd_diarClean)` Tables of the estimated treatment effects the primary Bangladesh outcomes (diarrheal disease and length-for-age Z-score), estimated using the functions in this package, are included at the end of this vignette.
...
If you wish to actually compare means between groups using a difference, prevalence ratio, or incidence ratio (depending on the outcome), use `washb_ttest()` (for unadjusted continuous outcomes), or `washb_mh()` (for unadjusted binary outcomes), or `washb_glm()` for unadjusted or adjusted generalized linear models.  

<br>
  
Using the cleaned Wash Benefits data, the mean and 95% Confidence interval for mother's age is calculated:
```{r, echo=FALSE, cache=TRUE, comment=NA}
MeanMomage<-washb_mean(Y=ad$momage,id=ad$clusterid,print=TRUE)
```
The `<-` command assigns the output of the `wahb_mean()` function to the new `MeanMomage` object. 
...
Load and merge the anthropometry and enrollment datasets. and clean as for the diarheal disease analysis shown above.
```{r, eval=F, comment=NA}
