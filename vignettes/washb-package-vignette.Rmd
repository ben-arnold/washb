...
```
On top of the function's auto-printed output, the washb_glm function contains a number of objects. For example, 'objectname'$RD returns the risk difference instead of the risk ratio for all covariates (not applicable when the glm  model is gaussian).
```{r, warning=FALSE, message=FALSE, eval=TRUE, cache=TRUE}
glm.C.W$RD
```
Other objects are:
`objectname'$fit' to return full glm output."
`objectname'$RR' to return the relative risks and 95% CI's of the full model, including pair-matched blocks."
`objectname'$cov' to return the variance-covariance matrix."
...
The "V" arguement can be used to create an interaction term with "tr", the treatment variable. V is a variable name of a variable that must be within the W matrix. Here is code for a subgroup analysis of the effect of water treatment on diarrheal disease, by sex. 
```{r, warning=FALSE, message=FALSE, cache=TRUE}
