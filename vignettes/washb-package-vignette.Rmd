...
```
The function washb_MH.pooled can also be used to calculate an unadjusted risk difference: 
```{r, warning=FALSE, message=FALSE, cache=TRUE}
...
-mention why the log-link is used

First, apply the washb_glm function to compare the water and control arms:
```{r, warning=FALSE, message=FALSE, eval=T, cache=TRUE}
...
The "V" arguement can be used with multilevel factors. Here is code for a subgroup analysis of the effect of water treatment on diarrheal disease, by target child (vs. sibling). 
```{r, warning=FALSE, message=FALSE, eval=F, cache=TRUE}
